...
class MySystem(LinuxX86System):
    ...
    def createCacheHierarchy(self):
        ...
        for i in xrange(np):
            self.cpu[i].icache = L1ICache(self._opts)
            self.cpu[i].dcache = L1DCache(self._opts)

        # Connect the instruction, data, and MMU caches to the CPU
            self.cpu[i].icache.connectCPU(self.cpu)
            self.cpu[i].dcache.connectCPU(self.cpu)

        # Hook the CPU ports up to the membus
            self.cpu[i].icache.connectBus(self.membus)
            self.cpu[i].dcache.connectBus(self.membus)

        # Connect the CPU TLBs directly to the mem.
            self.cpu[i].itb.walker.port = self.membus.slave
            self.cpu[i].dtb.walker.port = self.membus.slave
...
#        self.cpu[0].createInterruptController()
#        self.cpu[0].interrupts[0].pio = self.membus.master
#        self.cpu[0].interrupts[0].int_master = self.membus.slave
#        self.cpu[0].interrupts[0].int_slave = self.membus.master
        for i in xrange(np):
        ...
        # Note: these are directly connected to the memory bus, not cached
            self.cpu[i].interrupts[0].pio = self.membus.master
            self.cpu[i].interrupts[0].int_master = self.membus.slave
            self.cpu[i].interrupts[0].int_slave = self.membus.master
